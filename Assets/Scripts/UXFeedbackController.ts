...
export class UXFeedbackController extends BaseScriptComponent {
    ...
    @input
    private MidasTouchVisual: SceneObject;
    @input
    @allowUndefined
    @hint(
        "Optional mesh visual for dwell indicator color feedback. If empty, uses first RenderMeshVisual on MidasTouchVisual.",
    )
    private midasTouchVisualMesh: RenderMeshVisual | undefined;
    @input
    @allowUndefined
    @hint(
        "Optional prefab shown while dwell is not ready. Falls back to color sphere when unassigned.",
    )
    private dwellNotReadyStatePrefab: ObjectPrefab | undefined;
    @input
    @allowUndefined
    @hint(
        "Optional prefab shown when dwell is ready to place a note. Falls back to color sphere when unassigned.",
    )
    ...
    @input
    @hint(
        "Seconds the dwell signal must remain changed before applying state (reduces jitter flicker).",
    )
    ...
    @allowUndefined
    @hint(
        "Optional loading indicator prefab. If empty, tries 3DLoadingIndicator__PLACE_IN_SCENE prefab.",
    )
    private loadingIndicatorPrefab: ObjectPrefab | undefined;
    @input
    @hint(
        "Offset from pin position in local space. Use negative Y to place below.",
    )
    ...
    @allowUndefined
    @hint(
        "Assign AreaManager to hide loading indicator when a new note widget is actually created.",
    )
    ...
    private onStart() {
        ...
        this.areaManagerForLoadingTracking?.onWidgetsUpdated.add((widgets) => {
            ...
            if (
                this.waitingForWidgetSpawn &&
                widgets.length >= this.expectedWidgetCountAfterSpawn
            ) {
    ...
    private onUpdate() {
        if (this.isIndexTipHighlightActive) {
            this.MidasTouchVisual.getTransform().setWorldPosition(
    ...
    private initializeLoadingIndicator(): void {
        ...
        if (!prefab) {
            try {
                ...
                print(
                    "[UXFeedbackController] Could not auto-load default loading indicator prefab.",
                );
            }
        }
        ...
        if (!prefab) {
            print(
                "[UXFeedbackController] Loading indicator is enabled but no prefab is assigned.",
            );
            return;
        }
        ...
        loadingTransform.setLocalScale(
            vec3.one().uniformScale(Math.max(0.01, this.loadingIndicatorScale)),
        );
    }
    ...
    private hideLoadingIndicator(): void {
        if (
            !this.isLoadingIndicatorActive &&
            !this.loadingIndicatorObject?.enabled
        ) {
            return;
        }
    ...
    private initializeDwellIndicatorMaterial(): void {
        this.dwellBaseMeshVisual =
            this.midasTouchVisualMesh ??
            this.MidasTouchVisual.getComponent("Component.RenderMeshVisual");
        if (
            !this.dwellBaseMeshVisual ||
            !this.dwellBaseMeshVisual.mainMaterial
        ) {
            print(
                "[UXFeedbackController] Dwell indicator mesh/material not found; color feedback disabled.",
            );
            return;
        }

        this.dwellIndicatorMaterial =
            this.dwellBaseMeshVisual.mainMaterial.clone();
    ...
    private initializeDwellStateVisuals(): void {
        ...
        this.dwellReadyStateObject = this.instantiateStatePrefab(
    ...
    private instantiateStatePrefab(
        ...
        stateTransform.setLocalScale(
            vec3.one().uniformScale(Math.max(0.01, scaleMultiplier)),
        );
    ...
    private applyDwellIndicatorState(isReady: boolean, force: boolean): void {
        if (
            !force &&
            this.lastDwellReadyVisualState !== undefined &&
        ...
        const activeStateObject = isReady
            ? this.dwellReadyStateObject
            : this.dwellNotReadyStateObject;
        const inactiveStateObject = isReady
            ? this.dwellNotReadyStateObject
            : this.dwellReadyStateObject;
        ...
        const pass = this.dwellIndicatorMaterial.mainPass as unknown as {
            [key: string]: vec4;
        };
        pass[this.midasTouchColorParameter] = isReady
            ? this.dwellReadyColor
            : this.dwellNotReadyColor;
    }
}

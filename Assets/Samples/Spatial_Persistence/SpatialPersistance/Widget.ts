...
import {clamp, mapValue} from "SpectaclesInteractionKit.lspkg/Utils/mathUtils"
...
export class Widget extends BaseScriptComponent {
  ...
  readonly onUpdateContent: PublicApi<void> = this.onUpdateContentEvent.publicApi()
  ...
  private onStart() {
    ...
    this.createEvent("UpdateEvent").bind(() => {
      if (this.doInterpolate) {
        let frac = mapValue(getTime(), this.interpolateStartTime, this.interpolateEndTime, 0, 1)
        if (frac >= 1.0) {
        ...
        const p = vec3.lerp(this.interpolatePosStart, this.interpolatePosEnd, frac)
        const rot = quat.slerp(this.interpolateRotStart, this.interpolateRotEnd, frac)
